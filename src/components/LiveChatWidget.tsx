...
import { MessageCircle, X, Send, User, Bot, Headphones, Star, History, ArrowLeft } from "lucide-react";
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (reply.includes("[TRANSFER_TO_HUMAN]")) {
        // Show transfer form instead of immediately transferring
                ...
                <button key={conv.id} onClick={() => openHistoryChat(conv)}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium">
                      {conv.is_ai ? "AI Support" : "Live Agent"}
                    </span>
                    <span className={cn("text-[10px] px-2 py-0.5 rounded-full",
                      conv.status === "active" ? "bg-success/20 text-success" : "bg-muted text-muted-foreground"
                    )}>{conv.status}</span>
                  </div>
